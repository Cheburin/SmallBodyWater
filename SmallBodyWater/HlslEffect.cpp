...
HRESULT CreateShaderFromFile(ID3D11Device* pd3dDevice, LPCWSTR pSrcFile, CONST D3D_SHADER_MACRO* pDefines,
	...
	BOOL bDumpShader = FALSE, bool withSo = FALSE);
	...
	BOOL bDumpShader, bool withSo)
{
	...
	if (ppShader)
	{
		...
		else if (!withSo && strstr(pProfile, "gs"))
		{
		...
		{
			D3D11_SO_DECLARATION_ENTRY SODeclarationEntry[6] =
			{
			...
			hr = pd3dDevice->CreateGeometryShaderWithStreamOutput(pShaderBlob->GetBufferPointer(), pShaderBlob->GetBufferSize(),
				SODeclarationEntry, _countof(SODeclarationEntry),
				NULL, 0, D3D11_SO_NO_RASTERIZED_STREAM, NULL, (ID3D11GeometryShader**)ppShader);
		}
...
public:
	//Constructor
	HlslEffect(ID3D11Device* device, std::map<const WCHAR*, EffectShaderFileDef>& fileDef, bool withSo){
		std::map<const WCHAR*, ID3D11DeviceChild*> shaders;

		std::for_each(fileDef.begin(), fileDef.end(), [this, withSo, device, &shaders](std::pair<const WCHAR*, EffectShaderFileDef> p) {
...
std::unique_ptr<DirectX::IEffect> createHlslEffect(ID3D11Device* device, std::map<const WCHAR*, EffectShaderFileDef>& fileDef, bool withSo){
	return std::unique_ptr<DirectX::IEffect>(new HlslEffect(device, fileDef, withSo));
}
