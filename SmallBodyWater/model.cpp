...
XMFLOAT3 position;
const D3D11_INPUT_ELEMENT_DESC PosTBNTex2d::InputElements[] =
{
...
std::unique_ptr<DirectX::ModelMeshPart> CreateTBNModelMeshPart(ID3D11Device* device, ID3D11Buffer** out_stream_buffer, std::function<void(std::vector<PosTBNTex2d> & _vertices, std::vector<UINT> & _indices)> createGeometry){
	std::vector<PosTBNTex2d> vertices;
	std::vector<UINT> indices;
	...
	modelMeshPArt->startIndex = 0;
	modelMeshPArt->vertexOffset = 0;
	modelMeshPArt->vertexStride = sizeof(PosTBNTex2d);
	modelMeshPArt->primitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	modelMeshPArt->indexFormat = DXGI_FORMAT_R32_UINT;
	modelMeshPArt->vbDecl = std::shared_ptr<std::vector<D3D11_INPUT_ELEMENT_DESC>>(
		new std::vector<D3D11_INPUT_ELEMENT_DESC>(
		&PosTBNTex2d::InputElements[0],
		&PosTBNTex2d::InputElements[PosTBNTex2d::InputElementCount]
		)
		);

	CreateBuffer(device, vertices, D3D11_BIND_FLAG(D3D11_BIND_VERTEX_BUFFER | D3D11_BIND_STREAM_OUTPUT), modelMeshPArt->vertexBuffer.ReleaseAndGetAddressOf());

	//CreateBuffer(device, vertices, D3D11_BIND_STREAM_OUTPUT, out_stream_buffer);
